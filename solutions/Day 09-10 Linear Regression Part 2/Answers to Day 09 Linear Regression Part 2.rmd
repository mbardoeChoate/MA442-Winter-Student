...
plot( USArrests$Murder ~ USArrests$Assault , 
     main = "Scatter Plot of Murder Rate by Assult Rate",
     xlab = "Assults (per 100k)",
...
*Your answer here.*
Yes because both variables are quantitative and there is a linear association.
...
```{r}
# Important to use the data=.... here so that when we use predict everything will work.
...
Predicted Murders = 0.04191 * Assaults + 0.63168

for each additional assault per 100k we would expect an additional 0.04191 murders per 100k

if there were no assaults we would expect 0.63168 murders per 100k
...
predict.data.frame <- data.frame( Assault = c(100, 230, 40 ) )

#Make the predictions

predict.data.frame$Predicted.Murder <- predict(linearMod.murder.assault, predict.data.frame)
...
```{r}
USArrests$Average.Murder <- mean(USArrests$Murder)

USArrests
USArrests [ , c("Murder", "Predictions", "Residuals", "Average.Murder")]

```


```{r}
USArrests$Avg.Murd.Resid <- USArrests$Murder - USArrests$Average.Murder
USArrests
USArrests [ , c("Murder", "Predictions", "Residuals", "Average.Murder", "Avg.Murd.Resid")]
```
...
```{r}
sum(USArrests$Residuals)
sum(USArrests$Avg.Murd.Resid)
```
...
sum(USArrests$Avg.Murd.Resid^2)
```

Our regression model has a much smaller sum of squared residuals than the just guessing model. It only has 
```{r}
sum(USArrests$Residuals^2)/sum(USArrests$Avg.Murd.Resid^2)
```
or 35% as much error as the original model.


That means that our model explains
```{r}
1- sum(USArrests$Residuals^2)/sum(USArrests$Avg.Murd.Resid^2)
```
64% of the variation of the response variable. 
...
Create a scatter plot of gas prices vs drivers killed. Make sure to put the explanitory and response variables on the correct axis.

```{r}
plot(df.seatbelts$DriversKilled~df.seatbelts$PetrolPrice,
     main = "Drivers Killed as Explained by Price of Petroleum",
     xlab = "Price of Petroleum",
     ylab = "Drivers Killed",
     col = "yellow",  
     bg = "pink",
     pch = 22,
...
The variables PetrolPrice and DriversKilled are both quantitative and the shape of the scatterplot is fairly linear so it is appropriate to use a linear model.
...
```{r}
linearMod.DriversKilled.PetrolPrice <- lm(DriversKilled ~ PetrolPrice, data = df.seatbelts)
print(linearMod.DriversKilled.PetrolPrice)
```


Not asked for but here is the scatter plot with the liner model drawn in: 
```{r}
#Scatter plot
plot( df.seatbelts$DriversKilled~df.seatbelts$PetrolPrice, 
     main = "Drivers Killed as Explained by Price of Gas with Residuals",
     xlab = "Price of Gas",
     ylab = "Drivers Killed",
     col = "yellow",  
     bg = "pink",
...
#Regression Line
abline(linearMod.DriversKilled.PetrolPrice,
       lwd = 3,
       col = "grey 59")

```

Not asked for but here is the scatter plot with the liner model and residuals drawn in: 

```{r}
#Create a vector of the predictions
df.seatbelts$Predictions <- predict(linearMod.DriversKilled.PetrolPrice)
...
#Residuals
segments(df.seatbelts$PetrolPrice, df.seatbelts$DriversKilled, # (x1, y1 )
         df.seatbelts$PetrolPrice, df.seatbelts$Predictions, #(x2, y2)
         col="light green",
         lwd = 2)
...
```{r}
df.seatbelts$Predictions <- predict(linearMod.DriversKilled.PetrolPrice)
df.seatbelts$Residuals <- resid(linearMod.DriversKilled.PetrolPrice)
head(df.seatbelts)
```
...
```{r}
plot(df.seatbelts$Residuals ~ df.seatbelts$Predictions,
     main = "Residual Plot",
     xlab = "Predicted Values",
     ylab = "Residuals",
     col = "dark green",  
...
Yes because the residual plot looks like a cloud.
...
```{r}
summary(linearMod.DriversKilled.PetrolPrice)
```
...
predicted Drivers.Killed = -805.86(PetrolPrice) + 206.31
...
No it could not be from random variation because the P value of Petrol Price is so low. We can also see this by the *** next to the P value. 
...
Gas price explains only 14.95% of the variation in drivers killed so it is not a great model.
