...
*Your answer here*
sqft and bathrooms appear correlated.


Let's make a scatter plot to see the relationship between price and square footage.
```{r}

plot( houses$Price ~ houses$Sqft, 
     main = "Scatter Plot of Price of Houses by Square Footage",
     xlab = "Square Feet",
     ylab = "Price (dollars)",
...
```{r}
#Run regression
lin.mod.Price.sqft <- lm( Price~Sqft, data = houses)

#Print summary
summary(lin.mod.Price.sqft)
...
24.66% of the variation in price is explained by variation in the square footage
...
**Question** What is the coefficient of square footage in the equation of our model? What does it mean in context of this problem?
 ...
 104.7 this number tells us that for every additional square foot we expect price to go up by 104.7 dollars.
...
```{r}
houses$predictions <- predict(multiMod.price.sqft.bed  )
houses$Residuals <- resid(multiMod.price.sqft.bed)
```
...
```{r}
plot(houses$Residuals ~houses$predictions)
