...
plot( trees$Height ~ trees$Girth, 
     main = "Scatter Plot of Height by Girth in Trees",
     xlab = "Girth (in)",
     ylab = "Height (ft)",
     col = "purple",  
     bg = "gold",
...
There is a moderate linear association.


Create a linear model and print out the results

```{r}
lin.Mod.height.girth <- lm( Height ~ Girth, data = trees)
lin.Mod.height.girth
```
...
predicted height = 1.054 * Girth + 62.031

slope: for each additional inch of girth we would expect an additional 1.054 feet of height.

intercept: if the girth of a tree is 0 inches than we would expect the tree to be 62.031 feet tall. This does not make sense in context.
...
predict.data.frame <- data.frame( Girth = c(5, 6, 7, 8, 9, 10, 11, 12, 13, 15.7) )

#Make the predictions

predict.data.frame$Predicted.Height <- predict(lin.Mod.height.girth, predict.data.frame)
